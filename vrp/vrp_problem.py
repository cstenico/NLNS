...
class VRPInstance():
    ...
    def create_initial_solution(self):
        ...
        mask[0] = False
        while mask.any():
            remaining_customers = np.where(mask)[0]
            closest_customer_indices = self.get_n_closest_locations_to(self.solution[-1][-1][0], mask, len(remaining_customers))

            for idx in closest_customer_indices:
                ...
                if self.demand[idx] < 0:
                    if cur_delivery_load + self.demand[idx] >= 0 and cur_distance + added_distance <= max_distance_km:
                        ...
                        #cur_load -= self.demand[idx]
                        cur_distance += added_distance
                        cur_delivery_load = cur_delivery_load + self.demand[idx]
